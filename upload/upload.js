...
const validateSoundmarks = (soundmarksArray) => {
  ...
  for (let item of soundmarksArray) {
    if (
      ...
      return {
        result: false,
        message: "A required property does not exist.",
        soundmarks: undefined,
      };
    }
    if (!item.trackLink.includes("https://soundcloud.com/")) {
      return {
        result: false,
        message: "One or more track links are invalid.",
        soundmarks: undefined,
      };
    }
    if (item.id.length < 10) {
      return {
        result: false,
        message: `One or more IDs are invalid (id: ${item.id}, track title: ${item.trackTitle}.`,
        soundmarks: undefined,
      };
    }
    for (let elem of item.timeStamp.split(":")) {
      if (isNaN(elem) || elem.length > 2) {
        return {
          result: false,
          message: "One or more timestamps are invalid.",
          soundmarks: undefined,
        };
      }
    }
  ...
  if (counter === soundmarksArray.length) {
    soundmarksArray.forEach(
      (x) => (x.createdAt = new Date(x.createdAt).getTime() / 1000),
    );
    return {
      result: true,
      message: "Import successful",
      soundmarks: soundmarksArray,
    };
  } else return false;
};
...
const tryImport = async (parsedFile) => {
  if (parsedFile) {
    ...
    if (result) {
      ...
      document.getElementById("upload_title").innerText =
        "Successfully imported soundmarks file!";
